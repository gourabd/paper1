...
St=optim(into,llh,n=n,newn=newn,S=S,denomdata=denomdata,no_mstrat=no_mstrat,offn=offn,newoffn=newoffn,offS=offS,data=data,no_strat=no_strat,q=q)
...
R[t,]=c(prop,St$par,St$value,nullf,tst,qchisq(0.95,length(into)),as.numeric(tst>qchisq(0.95,length(into))), betas)
}
R
}
R=sim(1000,15000,4,c(0),1,c(0.5))
write(apply(R,2,mean),"resfull1",append=TRUE)
write(apply(R,2,sd),"resfull1",append=TRUE)

R=sim(1000,30000,4,c(0,0),2,c(0.5,0.5))
