...
data=strata[,1:(q)]
no_strat=strata[,(q)+1]
offS=(split(seq(nrow(strata[,1:(q)])),as.list(data.frame(strcode))))
offn=names(offS)
...
St=optim(into,llh,n=n,newn=newn,S=S,denomdata=denomdata,no_mstrat=no_mstrat,offn=offn,newoffn=newoffn,offS=offS,data=data,no_strat=no_strat,q=q,control=list(maxit=5000))
...
R=sim(10,30000,4,c(0,0),2,c(0.5,0.5))
apply(R,2,mean)
