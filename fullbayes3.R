...
gradllh<-function(logbeta,n,newn,S,denomdata,no_mstrat,offn,newoffn,offS,data,no_strat,q)
{
 beta=rep(1,length(n))
 beta[match(newn,n)]=exp(logbeta)
workbeta=rep(1,length(denomdata[,1]))
 ...
 part1=rep(0,length(offn))
 part2b=rep(0,length(offn))

 offb=beta[match(offn,n)]
 for(i in match(newoffn,offn)){ part1[i]=no_strat[offS[[i]]]}
 part2a=offb
 for(i in match(newoffn,offn)){ part2b[i]=par[offS[[i]]]}
 
 grad=part1-(part2a*part2b)

 if (length(newoffn)!=length(offn)) grad=grad[-1]
 
 colnames(denomdata)=c(1:((3*q)+1))
 i=c((q+1):(3*q))
 up=aggregate(denom1,by=as.list(data.frame(pardata)),sum)
 colnames(up)=c((q+1):((3*q)+1))
 up=cbind(up,no_mstrat)
 comb=merge(up,denomdata,by=as.character(i))
 comb[,(3*q)+4]=comb[,(3*q)+3]*comb[,(3*q)]/comb[,(2*q)+1]
 offdata=data.frame(comb[,((3*q)+1):((3*q)+2)])
 par=(aggregate(comb[,10],by=as.list(offdata),sum))$x 
for(i in match(newoffn,offn)){ part2b[i]=par[offS[[i]]]}
...
sim<-function(l,num,mu,betas,q,pr)
{
...
into=c(0,0.5,0,0,0,0.5,0,0)

#St=optim(into,llh,gr=gradllh,method="BFGS",n=n,newn=newn,S=S,denomdata=denomdata,no_mstrat=no_mstrat,offn=offn,newoffn=newoffn,offS=offS,data=data,no_strat=no_strat,q=q,control=list(maxit=10000))
...
R=sim(10,30000,4,c(-0.5,0),2,c(0.5,0.5))
